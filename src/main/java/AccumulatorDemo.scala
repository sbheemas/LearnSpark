...
object AccumulatorDemo {
  ...
  def collectionAccDemo(sc: SparkContext): Unit = {
    ...
    def collAdd(x: String): Unit = {
      if (x.length > 6) { collectionAcc.add(x); println(x) }
    }
  ...
  def longAccDemo(sc: SparkContext): Unit = {
    ...
    val longAcc =
      sc.longAccumulator("Count of words having more than 5 letters")
    val rdd4 = sc.textFile("data\\lockdown.txt")

    def collAdd(x: String): Unit = {
      if (x.length > 5) { longAcc.add(1); println(x) }
    }
    rdd4.flatMap(x => x.split(",")).foreach(x => collAdd(x))
