...
object RetailDataSet {
  def main(args: Array[String]): Unit = {
    ...
    val kpi1 = spark.sql("select Country, sum(Quantity) from retail_data group by Country order by sum(Quantity) desc")

    //kpi1.show()

    kpi1.coalesce(1).write.mode(SaveMode.Append).csv("C:\\kpi1")

    val kpi2 = spark.sql("select CustomerID, sum(Quantity) from retail_data where CustomerID != null group by CustomerID order by sum(Quantity) desc")
