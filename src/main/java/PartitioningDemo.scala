...
object PartitioningDemo {
  def main(args: Array[String]): Unit = {
    ...
    val rdd3 = sc.parallelize(List((1,1),(3,2),(4,3),(2,1),(0,2),(20,3),(6,1),(5,2),(7,1),(8,3),(50,1)),2)

    println("Num of partitions: " + rdd3.getNumPartitions)

    val rdd4 = rdd3.partitionBy(new HashPartitioner(3))

    rdd4.mapPartitionsWithIndex(((i,words) => ( for ( w <- words ) yield ( w,i)))).collect.foreach(println)
    ...
    rdd5.mapPartitionsWithIndex((index,words) => {for (word <- words)yield(word,index)}).collect().foreach(println)
    ...
    rdd11.mapPartitionsWithIndex((index,words) => {for (word <- words)yield(word,index)}).foreach(println)
    ...
    rdd8.persist()
    rdd8.mapPartitions(words => { for (word <- words) yield (word,1)}).collect().foreach(println)

    println("mapPartitionsWithIndex.....")
    rdd8.mapPartitionsWithIndex((index,words) => {for (word <- words)yield(word,index)}).collect().foreach(println)
